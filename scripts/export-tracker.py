...
def main():
    ...
    # Column B (AISCE References) is dropped: it must never reach this repo.
    ...
    lines = ["# Term register export from the tracker sheet. GENERATED by export-tracker.py — do not edit.",
             "# term\tpublished\tstatus\tnote"]
    for r in rows[1:]:
        term = r[hdr["Term"]].strip()
        if not term:
            continue
        lines.append("\t".join([term,
                                "yes" if r[hdr["On Github Flag"]].strip() == "X" else "no",
                                r[hdr["Term Status"]].strip() or "unassessed",
                                r[hdr["Term Status Note"]].strip().replace("\t", " ")]))
    tsv.write_text("\n".join(lines) + "\n")
    print(f"wrote {tsv.relative_to(ROOT)} ({len(lines)-2} terms, AISCE column dropped)")
