...
def vocabulary(report):
    """Terms an outside list names that the corpus has never encountered.

    THE BLIND SPOT THIS EXISTS FOR (Sep 2026): the gap report counts unlinked
    prose mentions and the promise sweep reads Related-concepts bullets, so both
    measure demand generated by text the corpus ALREADY WROTE. Absence generates
    no demand — overfitting, underfitting and catastrophic forgetting were
    missing from 120 entries and scored near zero on both signals. Only a list
    from outside can see that, which is why vocabulary.txt must never be
    regenerated from the corpus.

    A hit is a prompt to judge, never an instruction to publish.
    """
    if not VOCABULARY.exists():
        report.append(f"vocab     {VOCABULARY.name} is missing — cannot check outside coverage")
        return
    ...
    for f in CONCEPTS.glob("*.md"):
        ...
        if m:
            known.update(a.strip().lower() for a in m.group(1).split(","))

    queued = set()
    if EXPORT.exists():
        for line in EXPORT.read_text().splitlines()[1:]:
            parts = line.split("\t")
            if parts:
                queued.add(parts[0].strip().lower())
    else:
        report.append(f"vocab     {EXPORT.name} is missing — tracker rows cannot be excluded, "
                      f"so hits below may already be queued")

    # Substring match against entry prose, so a term discussed under another
    # name still counts as encountered. Deliberately generous: this check is for
    # words the corpus has NEVER SEEN, and a false "covered" is cheaper than the
    # noise of flagging every term that appears in a sentence somewhere.
    prose = " ".join(f.read_text().lower() for f in _files())

    for raw in VOCABULARY.read_text().splitlines():
        term = raw.strip()
