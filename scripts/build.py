...
CRUMB_HOME = "[Applied AI Concepts](../README.md) › "  # every entry opens with a generated breadcrumb
...
def check(entries):
    ...
    # 8. breadcrumbs
    problems += breadcrumb_problems(entries, read_readme_rows()[2])
    return problems
...
def write(entries):
    ...
    # breadcrumbs — first line of every entry, derived from its catalog section
    _, _, rorder = read_readme_rows()
    cat_of = {s: c for c, slugs in rorder.items() for s in slugs}
    crumbs = 0
    for e in entries:
        if e["slug"] not in cat_of:
            continue
        path = CONCEPTS / f"{e['slug']}.md"
        text = path.read_text(encoding="utf-8")
        new = with_breadcrumb(text, breadcrumb(e, cat_of[e["slug"]]))
        if new != text:
            path.write_text(new, encoding="utf-8")
            crumbs += 1
    if crumbs:
        changed.append(f"breadcrumbs ({crumbs} entries)")
...
def breadcrumb(e, cat):
    """Home › Category › Term, derived from the entry's section in glossary/categories.md."""
    return f"{CRUMB_HOME}[{cat}](../glossary/categories.md#{gh_anchor(cat)}) › {e['term']}"


def with_breadcrumb(text, crumb):
    """Entry text with `crumb` as the first line after the meta block (replacing any old one)."""
    head, sep, body = text.partition("\n-->\n")
    if not sep:
        return text
    lines = body.split("\n")
    if lines and lines[0].startswith(CRUMB_HOME):
        lines = lines[1:]
        while lines and not lines[0].strip():
            lines = lines[1:]
    return head + sep + crumb + "\n\n" + "\n".join(lines)


def breadcrumb_problems(entries, rorder):
    cat_of = {s: c for c, slugs in rorder.items() for s in slugs}
    out = []
    for e in entries:
        cat = cat_of.get(e["slug"])
        if not cat:
            continue  # already reported by the categories round-trip
        text = (CONCEPTS / f"{e['slug']}.md").read_text(encoding="utf-8")
        if with_breadcrumb(text, breadcrumb(e, cat)) != text:
            out.append(f"breadcrumb: {e['slug']} missing or stale (run `build.py write`)")
    return out
