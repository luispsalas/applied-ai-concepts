...
**The only check that can catch a wrong *referent*.** It extracts every `| SRC-NNN | …` row from the files you name and compares each against how the same ID is rendered elsewhere in the corpus, reporting `DIFFERS` (exit 1) when it points at a **different document**: a different URL, or a different title when no form carries a link.
...
**A `DIFFERS` line lists every URL the corpus uses for that ID and recommends none of them.** Confirm the right one against the registry. Earlier versions suggested aligning to the most common form, and on the two occasions that was checked, the most common form was the wrong one and the minority matched the registry. **A count shows agreement, not correctness.**

**Wording-only differences are counted, not listed.** A citation that names the same URL differently (a longer author list, a dropped subtitle) is summarized as `wording-only` and does not fail the run; pass `--verbose` to see them. Listing them on every publish made this a check nobody read: before the change it failed on 86 of 154 entries, all for wording, and it now fails on none.
...
**Everything else is left alone deliberately.** Two citations sharing a URL cite the same document however differently they name it, and many IDs carry more than one rendered form for purely presentational reasons — a dropped subtitle, an expanded abbreviation, `&` for `;` (`citecheck.py --all` gives the current count). **Normalizing those is not worth a 130-file diff**, and aligning them to whichever form is most common would ratify whichever error happens to be most common. The filter's calibration is the evidence — treating any title difference as substantive flagged 26 IDs, prefix-tolerance brought it to 13, keying on URL gives the handful that are real.
...
- Reverse index (`col L`) in **both** directions — regenerate with `build.py report` and compare; do not hand-edit. **`export-tracker.py` now checks this on every export** (read-only): it lists any source whose column L disagrees with the pages that actually cite it, any cited source not marked `Active`, and any `Active` source cited nowhere with no use outside the wiki noted. Uncited `Pending`/`Backlog` sources are the intended queue and appear only as a count. Findings are printed, never written to the repo. Fix flagged rows only, and read each back by ID.
- `Outstanding` ↔ `Sources` promotion drift.
- **Year convention: record the year of the PUBLISHED version.** When a paper has both an arXiv preprint and a peer-reviewed version (conference proceedings or journal), the year column and every rendering carry the **venue's** year, and the venue is named in the registry note. A preprint that was never published carries its **first-submission** year, never the date of a later revision. *(Decided Sep 2026, after a registry-wide sweep found 14 rows already following it and one error: a 2022 NeurIPS paper recorded under the year of its last arXiv revision.)*
- **arXiv year sweep.** Query the arXiv API for every arXiv-linked row and list the rows whose year differs from the first-submission year. Each one must be either a venue year named in its note or a defect. **No other check catches this:** a year that is wrong the same way everywhere passes the link, ID, schema and citation-form checks alike.
