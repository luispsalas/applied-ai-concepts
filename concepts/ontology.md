...
**The ladder has a rung above the ontology: the language the ontology is written in.** An ontology describes a domain. A **metamodel** describes the ontologies themselves, fixing which kinds of element any ontology in that language may contain. OWL 2 has one. The OWL 2 Structural Specification defines the conceptual structure of every OWL 2 ontology, independent of syntax, and it is written in UML with notation *"compatible with the Meta-Object Facility (MOF)"*. MOF is the OMG standard that provides *"the basis for metamodel definition in the OMG family of MDA languages"*. **So the argument below about what a schema makes unsayable applies one level up**: the language bounds which kinds of statement an ontology can make at all. ⚠️ Inside OWL 2 the word *metamodeling* means something narrower: using one identifier for more than one type of entity, so that it *"can be used to state facts about classes and properties themselves."*
...
7. **Above the ontology sits the metamodel**, the language it is written in, which bounds what any ontology can express.
8. **Ontologies ossify.** Early choices outlive their rationale because changing them breaks dependents.
9. **The computing sense is standardized** (W3C), which is what separates it from the philosophical sense sharing the word.
...
| SRC-370 | World Wide Web Consortium (W3C) — *OWL 2 Web Ontology Language Structural Specification and Functional-Style Syntax (Second Edition)* (W3C Recommendation, 11 December 2012) · [link](https://www.w3.org/TR/owl2-syntax/) | The metamodel rung, from the standard itself: the structural specification defines the conceptual structure of OWL 2 ontologies independent of any syntax, and is written in UML with notation compatible with MOF. Also the source of OWL's narrower internal sense of *metamodeling* (one identifier used for more than one type of entity), which the entry flags as a name collision. Wording verified against the raw specification. |
| SRC-371 | Object Management Group (OMG) — *Meta Object Facility (MOF) Specification, Version 2.5.1* (formal specification, October 2016) · [link](https://www.omg.org/spec/MOF/2.5.1/About-MOF) | The standard meaning of *metamodel*: MOF provides the basis for metamodel definition in the OMG family of MDA languages. Cited for that definitional role only. ⚠️ A software-modeling standard, not an AI source: the entry uses it to name the rung, not to make any AI claim. |
