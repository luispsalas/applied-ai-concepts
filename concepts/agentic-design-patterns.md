...
**Other vendors' catalogs converge on the same shapes under different names.** Google's *Introduction to Agents* describes a Coordinator pattern, which *"introduces a 'manager' agent that analyzes a complex request, segments the primary task, and intelligently routes each sub-task to the appropriate specialist agent"*, alongside Sequential, Iterative Refinement and Human-in-the-Loop patterns. OpenAI's guide describes a manager pattern and a decentralized pattern in which agents hand work off to one another, and Microsoft's architecture guidance lists sequential, concurrent, group chat, handoff and magentic orchestration. **The names differ far more than the shapes do**, which is a reason to reason from the shape and treat any single catalog's names as one vocabulary among several.
...
| SRC-372 | OpenAI — *A practical guide to building agents* (2025) · [link](https://cdn.openai.com/business-guides-and-resources/a-practical-guide-to-building-agents.pdf) | A second vendor's pattern catalog (manager and decentralized patterns) and its standing advice to maximize a single agent's capabilities first, with an incremental approach. ⚠️ Vendor-authored. |
| SRC-373 | Microsoft — *AI Agent Orchestration Patterns* (Azure Architecture Center, 2026) · [link](https://learn.microsoft.com/en-us/azure/architecture/ai-ml/guide/ai-agent-design-patterns) | A third vendor's catalog (sequential, concurrent, group chat, handoff, magentic orchestration) and the advice to use the lowest level of complexity that reliably meets requirements. ⚠️ Vendor-authored. |
| SRC-374 | Blount, A.; Gulli, A.; Saboo, S.; Zimmermann, M.; Vuskovic, V. (Google) — *Introduction to Agents* (Kaggle whitepaper, updated May 2026) · [link](https://www.kaggle.com/whitepaper-introduction-to-agents) | A fourth vendor's catalog: Coordinator, Sequential, Iterative Refinement and Human-in-the-Loop patterns, showing the shapes converge across vendors under different names. ⚠️ Vendor-authored. |
