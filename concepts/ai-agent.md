...
- **Feedback loops** — agents observe the results of their actions and revise their plan accordingly (canonical pattern: Reason → Act → Observe, repeated until goal is reached or a stopping condition is met). OpenAI's guide describes the same structure as a *run*: *"typically implemented as a loop that lets agents operate until an exit condition is reached"*
...
| SRC-372 | OpenAI — *A practical guide to building agents* (2025) · [link](https://cdn.openai.com/business-guides-and-resources/a-practical-guide-to-building-agents.pdf) | The agent run as a loop that continues until an exit condition is reached; agent components (model, tools, instructions); and when a deterministic solution suffices instead. ⚠️ Vendor-authored. |
| SRC-374 | Blount, A.; Gulli, A.; Saboo, S.; Zimmermann, M.; Vuskovic, V. (Google) — *Introduction to Agents* (Kaggle whitepaper, updated May 2026) · [link](https://www.kaggle.com/whitepaper-introduction-to-agents) | Defines an agent as a complete application that makes plans and takes actions to achieve goals, not a model in a static workflow; a taxonomy of agentic systems from Level 0 to Level 4. ⚠️ Vendor-authored. |
...
*Last updated: v1.3 · September 2026*
