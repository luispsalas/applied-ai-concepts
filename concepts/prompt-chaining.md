...
**It is the simplest workflow arrangement, and it sits at the bottom of the autonomy ladder.** Anthropic's *Building Effective Agents* lists prompt chaining first among its workflow patterns, before routing, parallelization, orchestrator–workers and evaluator–optimizer — and the standing advice across that literature is to prefer the simplest arrangement that works ([agentic design patterns](agentic-design-patterns.md), [orchestration](orchestration-ai-systems.md)). **The path is fixed in code; the model fills the steps but does not choose them**, which is exactly what makes a chain enumerable, testable and auditable in a way an agentic arrangement is not. **The same arrangement appears in other vendors' guidance:** Microsoft's architecture guidance calls it sequential orchestration, *"also known as a pipeline, prompt chaining, or linear delegation"*, and Google's *Introduction to Agents* calls it the Sequential pattern, where *"the output from one agent becomes the direct input for the next."*
...
| SRC-373 | Microsoft — *AI Agent Orchestration Patterns* (Azure Architecture Center, 2026) · [link](https://learn.microsoft.com/en-us/azure/architecture/ai-ml/guide/ai-agent-design-patterns) | Names the same arrangement sequential orchestration, also known as a pipeline, prompt chaining or linear delegation, for step-by-step processing where each stage builds on the previous one. ⚠️ Vendor-authored. |
| SRC-374 | Blount, A.; Gulli, A.; Saboo, S.; Zimmermann, M.; Vuskovic, V. (Google) — *Introduction to Agents* (Kaggle whitepaper, updated May 2026) · [link](https://www.kaggle.com/whitepaper-introduction-to-agents) | Describes it as the Sequential pattern, a digital assembly line where one agent's output is the next agent's input. ⚠️ Vendor-authored. |
